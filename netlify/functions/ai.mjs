const getKey = (name) => Netlify.env.get(name) || '';
...
export default async (request) => {
  ...
  try {
    ...
    if (input.action === 'transcribe') {
      ...
      const transcript = await transcribeAudio(input.audioData || '', input.audioMime || 'audio/webm', keys.openai);
    ...
    let reply = '';
    if (provider === 'gemini') reply = await callGemini(input, keys.gemini);
    if (provider === 'claude') reply = await callClaude(input, keys.claude);
    if (provider === 'openai') reply = await callOpenAI(input, keys.openai);
...
async function callClaude(input, key) {
  ...
  for (const msg of input.messages || []) {
    ...
    if (msg.image) {
      content.push({
        type: 'image',
        source: { type: 'base64', media_type: msg.image.mimeType, data: msg.image.data },
      });
    }
...
async function transcribeAudio(base64Audio, mimeType, key) {
